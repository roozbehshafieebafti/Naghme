...
class persian_calendar
{
     ...
     * @param   int     $timestamp  (Optional) Timestamp to format
     * @param   boolean $persian    (Optional) Convert digits to Persian or not
     * @return  string  Formatted local time/date according to locale settings
     */
    public function strftime($format, $timestamp='', $persian = false)
    {
        if($timestamp==''){
            $timestamp = mktime();
        }
     ...
     */
    public function date($format, $timestamp='', $persian = false)
    {
        if($timestamp==''){
            $timestamp = mktime();
        }
     ...
     * @author Omid Mottaghi
     * @access public
     * @param   int $hour   (Optional) hour
     * @param   int $minute (Optional) minute
     * @param   int $second (Optional) second
     * @param   int $month  (Optional) month
     * @param   int $day    (Optional) day
     * @param   int $year   (Optional) year
     * @param   int $is_dst (Optional) is daylight savings time set?
    ...
    /**
     * Convert all English numbers to Persian
     *
     * @author  Omid Mottaghi
     * @access public
    ...
    /**
     * Convert a compelete Gregorian date to Persian date, all date slices are separated by "/" by default
     *
     * @author  Omid Mottaghi
     * @access public
     * @param   string  $date           Gregorian date
     * @param   string  $separator      (Optional) Separator of day and month and year of Gregorian date
     * @param   string  $output_format  (Optional) Output format of Gregorian date
     * @param   boolean $persian        (Optional) Convert digits to Persian or not
     * @return  string  Gregorian date
     */
    public function date_g2p($date, $separator = '/', $output_format = 'd/m/Y', $persian = false)
    {
        $date = explode($separator, $date);
        $date = mktime(0, 0, 0, $date[1], $date[0], $date[2]);
        $date = system::persian_date_utf($output_format, $date, $persian);
        return $date;
    }

    /**
     * Convert a compelete Persian date to Gregorian date, all date slices are separated by "/" by default
     *
     * @author  Omid Mottaghi
     * @access public
     * @param   string  $date           Persian date with Latin numbers
     * @param   string  $separator      (Optional) Separator of day and month and year of Persian date
     * @param   string  $output_format  (Optional) Output format of Gregorian date
     * @return  string  Gregorian date
     */
    public function date_p2g($date, $separator = '/', $output_format = 'd-m-Y')
    {
        $date = explode($separator, $date);
        $date = system::persian_mktime(0, 0, 0, $date[1], $date[0], $date[2]);
        $date = date($output_format, $date);
        return $date;
    }
}
